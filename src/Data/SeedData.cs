...
public static class SeedData
{
    public static async Task InitializeAsync(IServiceProvider serviceProvider)
    {
        ...
        // Create admin user
        var adminEmail = "admin@appleshop.com";
        ...
        if (adminUser == null)
        {
            ...
            var result = await userManager.CreateAsync(adminUser, "Shopapple@2026");
            if (result.Succeeded)
            {
        ...
        {
            // Update admin password even if user already exists
            var token = await userManager.GeneratePasswordResetTokenAsync(adminUser);
            var passwordResult = await userManager.ResetPasswordAsync(adminUser, token, "Shopapple@2026");
